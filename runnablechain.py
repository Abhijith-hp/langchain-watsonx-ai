...
sentiment_llm_chain = LLMChain(llm=llama_llm,prompt=sentiment_prompt,output_key="sentiment")
summary_llm_chain = LLMChain(llm=llama_llm,prompt=summary_prompt,output_key="summary")
response_llm_chain = LLMChain(llm=llama_llm,prompt=response_prompt,output_key="response")

overall_chain = SequentialChain(
  
    chains=[sentiment_llm_chain, summary_llm_chain, response_llm_chain],
...
print(overall_chain.invoke(input = {"review":positive_review}))
print(overall_chain.invoke(input = {"review":negative_review}))
...
sentiment_chain = (
    sentiment_prompt | llama_llm | StrOutputParser() )

summary_chain = (
    summary_prompt | llama_llm | StrOutputParser() )

response_chain = (
    response_prompt | llama_llm | StrOutputParser() )
...
def test_chains(review):
    ...
    print(lcel_chain.invoke({"review": review}))
